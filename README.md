...
| `BACKGROUND` | `glass` | 背景：`glass` 半透明毛玻璃、`system` 完全交给系统材质、`solid` 不透明主题底色、`clear` 全透明 |
| `LATENCY_STYLE` | `chip` | 延迟数值的画法：`chip` 主题色当底、数字用正文色；`text` 数字直接染成延迟色 |
...
- **毛玻璃是近似的，能不能真透明取决于 Egern。** DSL 没有模糊原语 —— 真正的模糊由系统提供，
  小组件本来就浮在一层材质上。`glass` 做的是在那层材质之上压一层半透明灰渐变（上深下浅）。
  如果 Egern 自己铺了一层不透明的容器底，那么无论 alpha 调到多低都透不出壁纸；
  `BACKGROUND: "clear"`（全透明）就是用来验证这一点的：能看见壁纸说明可以做真透明，
  仍是一块浅灰说明做不到。
- **延迟数值默认画成色块**（`chip`）：主题的延迟色当底色、数字本身用正文色。
  主题的 `--latency-good` / `--latency-moderate` 是画在深色卡片上的黄绿，直接当文字色摆在
  浅色小组件上根本看不清；改成底色后档位照样一眼可辨，可读性却不再取决于那几档颜色。
  想回到「数字直接染色」用 `LATENCY_STYLE: "text"`。
