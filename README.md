...
全程在 Egern App 里点，**不用编辑配置文件**。四步：建脚本 → 建小组件 → 加到主屏 → 选中它。
...
小尺寸显示的是**单台**机器。想固定看某一台，回小组件画廊再点一次 **+**：

| 字段 | 填写内容 |
| --- | --- |
| 名称 | `HK-01` |
| 脚本名称 | 还是选 `cfsm-status` |
| 环境变量 | `NODE` = `hk-01`（机器 id 或名称的一段） |

**脚本不用再建一遍**，多个小组件共用同一个脚本。主屏上按第 3 步各加一次、分别选中即可。

### 环境变量填在哪

脚本和小组件两处都能填环境变量，**两边会合并**。所以：

- 公共的（`API_BASE`、`CARRIER`）填在**脚本**上，只填一次；
- 每个小组件各自不同的（`NODE`、`TITLE`、`ROWS`）填在**那个小组件**上。
...
| 画廊的 **+** 里选不到脚本 | 第 1 步的脚本没建成，或类型不是 `generic` | 回 **工具** → **脚本** 检查名称与类型 |
| 未配置 API_BASE | 环境变量没读到 | 检查 `API_BASE` 的拼写（全大写），以及是填在脚本还是这个小组件上 |
| 无法连接 xxx | 域名不通或超时 | 用手机浏览器开一下 `https://你的地址/api/servers`，确认能出 JSON；再把脚本的**超时**调大 |
