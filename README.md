...
| 尺寸 | 内容 |
|------|------|
| 中尺寸（systemMedium） | 旗帜 + 名称 + 在线徽章；CPU/内存、磁盘/负载 双列指标 + 分段进度条；在线时长与延迟底行 |
| 大尺寸（systemLarge） | 中尺寸全部内容，另加：剩余流量（含套餐分段进度条，无限流量显示 ∞）、延迟/丢包率双列 + 分段条、在线时长与到期时间底行 |

- 自适应浅色/深色模式
- 离线自动降级：徽章显示「x 分钟前」、进度条变灰，保留最后已知数据
- 延迟/丢包取电信、联通、移动、境外四路的均值/最大值
- 点击小组件跳转到监控站点
