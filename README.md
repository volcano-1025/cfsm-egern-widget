...
| `BACKGROUND` | `glass` | 背景：`glass` 半透明毛玻璃、`system` 完全交给系统材质、`solid` 不透明主题底色 |
...
- **毛玻璃是近似的。** DSL 没有模糊原语 —— 真正的模糊由系统提供，小组件本来就浮在一层材质上。
  `glass` 做的是在那层材质之上压一层半透明灰渐变（上深下浅），凑出原生电池、天气那类小组件的
  质感。副作用是浅色下整体压暗一点，延迟那几档黄绿（`#9fe339` / `#cbd83a`）会更不容易看清；
  觉得糊就换 `BACKGROUND: "system"`（完全交给系统材质，对比度最好）或 `"solid"`（不透明底色）。
