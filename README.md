...
- 延迟的**深色**配色与主题逐值一致，**浅色被整体压深了一档**：主题那几个色画在深色卡片和
  色带上，而小组件浅色态是接近纯白的系统材质，原来的黄绿（`#9fe339` / `#cbd83a`）在白底上
  几乎看不见。色相顺序不变，只是明度降下来。
- 进度条颜色沿用主题：**蓝=CPU、紫=内存、橙=磁盘**（大尺寸标题栏有图例）。
- **行高是写死的。** 小组件在 Mac / iPad 上比 iPhone 高，不定高的话多出来的竖直空间会被
  平摊到每一行，节点之间拉出巨大的空隙。定高之后节点列表始终贴着顶部排，空间富余就留在底部。
